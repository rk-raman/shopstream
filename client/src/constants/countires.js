...
export const getCountryShortCode = (countryName) => {
  const country = countries.find(
    (country) => country.countryName === countryName
  );
  return country ? country.countryShortCode : null;
};

export const getStateShortCode = (countryName, stateName) => {
  const country = countries.find(
    (country) => country.countryName === countryName
  );
  if (country) {
    const state = country.regions.find((state) => state.name === stateName);
    return state ? state.shortCode : null;
  }
  return null;
};
...
export const getStateOptions = (countryName = "India") => {
...
export function getMobileCodeByCountryName(countryName) {
  const formattedName =
    countryName.charAt(0).toUpperCase() + countryName.slice(1).toLowerCase(); // Normalize case
  return countryMobileCodes[formattedName] || "";
}
